...
class util
{
    ...
    public static function var_dump_plain(
        ...
        if (is_bool($var)) {
            $html .=
                '<span style="color:#588bff;">bool</span><span style="color:#999;">(</span><strong>' .
                ($var ? 'true' : 'false') .
                '</strong><span style="color:#999;">)</span>';
        } elseif (is_int($var)) {
            $html .=
                '<span style="color:#588bff;">int</span><span style="color:#999;">(</span><strong>' .
                $var .
                '</strong><span style="color:#999;">)</span>';
        } elseif (is_float($var)) {
            $html .=
                '<span style="color:#588bff;">float</span><span style="color:#999;">(</span><strong>' .
                $var .
                '</strong><span style="color:#999;">)</span>';
        } elseif (is_string($var)) {
            $html .=
                '<span style="color:#588bff;">string</span><span style="color:#999;">(</span>' .
                strlen($var) .
                '<span style="color:#999;">)</span> <strong>"' .
                self::htmlentities($var) .
                '"</strong>';
        } elseif (is_null($var)) {
            $html .= '<strong>NULL</strong>';
        } elseif (is_resource($var)) {
            $html .=
                '<span style="color:#588bff;">resource</span>("' .
            ...
            if ($depth > 0) {
                foreach ($done as $prev) {
                    if ($prev === $var) {
                        $html .=
                            '<span style="color:#588bff;">array</span>(' .
                            count($var) .
                            ') *RECURSION DETECTED*';
                        return $html;
                    }
            ...
            $html .=
                (!empty($var)
                    ? ' <img id="' .
                        $uuid .
                        '" src="data:image/gif;base64,R0lGODlhAQABAAD/ACwAAAAAAQABAAACADs%3D" onclick="javascript:colToggle(this.id);" /><script>setImg("' .
                        $uuid .
                        '",' .
                        $setImg .
                        ',1);</script>'
            ...
            foreach ($done as $prev) {
                if ($prev === $var) {
                    $html .=
                        '<span style="color:#588bff;">object</span>(' .
                        get_class($var) .
                        ') *RECURSION DETECTED*';
                    return $html;
                }
            ...
            $html .=
                ...
                '",' .
                $setImg .
                ',1);</script><span style="color:#588bff;">object</span>(' .
                get_class($var) .
                ') <span id="' .
                $uuid .
                '-collapsable" style="' .
            ...
            foreach ($varArray as $key => $value) {
                if (substr($key, 0, 2) == "\0*") {
                    ...
                    $key =
                        'private:' .
                        substr($key, 1, strpos(substr($key, 1), "\0")) .
                        ':' .
                        substr($key, strpos(substr($key, 1), "\0") + 2);
                    $varArray[$key] = $value;
                }
            ...
            foreach ($varArray as $key => $value) {
                ...
                $value = explode(
                    '<br />',
                    self::var_dump_plain($value, $expLevel, $depth + 1, $done)
                );
    ...
    public static function slugify($string, $separator = '-', $css_mode = false)
    {
        ...
        $slug = preg_replace(
            '/([^a-z0-9]+)/',
            $separator,
            strtolower(self::remove_accents($string))
        );
    ...
    public static function size_format($bytes, $decimals = 0)
    {
        ...
        if ($bytes < 1024) {
            ...
            return number_format($bytes / pow(1024, 2), $decimals, '.', '') .
                ' MiB';
        } elseif ($bytes < pow(1024, 4)) {
            return number_format($bytes / pow(1024, 3), $decimals, '.', '') .
                ' GiB';
        } elseif ($bytes < pow(1024, 5)) {
            return number_format($bytes / pow(1024, 4), $decimals, '.', '') .
                ' TiB';
        } elseif ($bytes < pow(1024, 6)) {
            return number_format($bytes / pow(1024, 5), $decimals, '.', '') .
                ' PiB';
        } else {
            return number_format($bytes / pow(1024, 5), $decimals, '.', '') .
                ' PiB';
    ...
    public static function add_query_arg($newKey, $newValue = null, $uri = null)
    {
        ...
        if (is_array($newKey)) {
            ...
            // Was the URL passed as an argument?
            $uri = is_null($uri)
                ? self::array_get($_SERVER['REQUEST_URI'], '')
                : $uri;
        }
    ...
    {
        if (is_array($keys)) {
            return self::add_query_arg(
                array_combine($keys, array_fill(0, count($keys), null)),
                $uri
            );
        }

        return self::add_query_arg([$keys => null], $uri);
    }
     ...
     * @return string
     */
    public static function http_build_url(
        $url,
        $parts = [],
        ...
        if ($flags & self::HTTP_URL_STRIP_ALL) {
            $flags |=
                self::HTTP_URL_STRIP_USER |
                self::HTTP_URL_STRIP_PASS |
                self::HTTP_URL_STRIP_PORT |
                self::HTTP_URL_STRIP_PATH |
                self::HTTP_URL_STRIP_QUERY |
                self::HTTP_URL_STRIP_FRAGMENT;
    ...
    public static function str_to_bool($string, $default = false)
    {
        ...
        $no_words =
            'no*|no way|nope|nah|na|never|absolutely not|by no means|negative|never ever|false|f|off|0|non|faux';
    ...
    public static function downcode($text, $language = '')
    {
        ...
        if (self::seems_utf8($text)) {
            ...
            $search =
                "\x80\x83\x8a\x8e\x9a\x9e\x9f\xa2\xa5\xb5\xc0\xc1\xc2\xc3\xc4\xc5\xc7\xc8\xc9\xca\xcb\xcc\xcd";
            $search .=
                "\xce\xcf\xd1\xd2\xd3\xd4\xd5\xd6\xd8\xd9\xda\xdb\xdc\xdd\xe0\xe1\xe2\xe3\xe4\xe5\xe7\xe8\xe9";
            $search .=
                "\xea\xeb\xec\xed\xee\xef\xf1\xf2\xf3\xf4\xf5\xf6\xf8\xf9\xfa\xfb\xfc\xfd\xff";
            $text = strtr(
                $text,
                $search,
    ...
    public static function number_to_word($number)
    {
        ...
        if ($number[0] == '0') {
            ...
            foreach ($groups as $group) {
                ...
                $groups2[] = self::numberToWordThreeDigits(
                    $group[0],
                    $group[1],
                    $group[2]
                );
            ...
            for ($z = 0; $z < count($groups2); $z++) {
                if ($groups2[$z] != '') {
                    $output .=
                        $groups2[$z] .
                        self::numberToWordConvertGroup($length - $z);
                    $output .=
                        $z < $length &&
    ...
    public static function secure_random_string($length = 16)
    {
        if (function_exists('openssl_random_pseudo_bytes')) {
            ...
            if ($bytes === false) {
                throw new \LengthException(
                    '$length is not accurate, unable to generate random string'
                );
            }
     ...
     * @return bool
     */
    public static function match_string(
        $pattern,
        $string,
        $caseSensitive = true
    ) {
    ...
    protected static function linkifyRegex($text)
    {
        ...
        $url_replace =
            '$1$4$7$10$13<a href="$2$5$8$11$14">$2$5$8$11$14</a>$3$6$9$12';
     ...
     * @return string
     */
    public static function limit_characters(
        $string,
        $limit = 100,
        $append = '...'
    ) {
    ...
    public static function ordinal($number)
    {
        ...
        $ext =
            abs($number) % 100 < 21 && abs($number) % 100 > 4
                ? 'th'
                : ($test_c < 4
                    ? ($test_c < 3
                        ? ($test_c < 2
    ...
    public static function array_flatten(array $array, $preserve_keys = true)
    {
        ...
        array_walk_recursive($array, function ($value, $key) use (
            &$flattened,
            $preserve_keys
        ) {
     ...
     * @return array
     */
    public static function array_pluck(
        array $array,
        $field,
        $preserve_keys = true,
        $remove_nomatches = true
    ) {
        $new_list = [];
     ...
     */
    public static function array_search_deep(
        array $array,
        $search,
        $field = false
    ) {
     ...
     * @return array
     */
    public static function array_map_deep(
        array $array,
        $callback,
        $on_nonscalar = false
    ) {
